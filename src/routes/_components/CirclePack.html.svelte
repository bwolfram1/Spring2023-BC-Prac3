  ...
  <div class="circle-pack" data-has-parent-key="{parentKey !== undefined}">
    {#each descendants as d}
      ...
      >
        <div
          class="circle"
          style="left:{d.x}px;top:{d.y}px;width:{d.r * 2}px;height:{d.r * 2}px;background-color:{d.data.data[fillKey]};border: {strokeWidth}px solid {stroke};"
          on:mouseover={(e) => dispatch("mousemove", { e, props: d })}
          on:focus={(e) => dispatch("mousemove", { e, props: d })}
          on:mouseout={(e) => dispatch("mouseout")}
          on:blur={(e) => dispatch("mouseout")}
          on:mousedown={(e) => dispatch("mousedown", {e, props:d})}
          />
          <div
            class="text-group"
            style="
